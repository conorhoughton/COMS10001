...
actual shortest route is given in Fig.~\ref{fig:shortest}. Now if you
start at ($a$) you can work out directly the distance using one edge
to all the nodes adjacent to $(a)$, this is given in
Fig.~\ref{fig:near_a}. Now the distance of ten to ($b$) is not the
actual shortest distance, there is a shorter path that goes by way of
($c$), the node marked four; this would give a distance of six. Hence,
we should now chose ($c$) and update the distances to the other nodes,
if going by way of ($c$) is shorter then this new distance should
replace the old one.
...
\begin{center}
\begin{tikzpicture}
\node[draw,thick,circle](a){$a$};
\node[draw,circle, below      =2cm of a](c){$4$};
\node[draw,circle, left = 2cm of a](d){$11$};
\node[draw,circle, below = 2 cm of d](b){$10$};
\node[draw,fill,circle, below left = 2 cm of b](e){};
\path (a) edge node[above]{10} (b);
\path (a) edge node[right]{4} (c);
\path (b) edge node[left]{3} (d);
\path (a) edge node[above]{11} (d);
...
\end{tikzpicture}
\end{center}
\caption{This shows the distance to the nodes adjacent to ($a$) directly along one edge, the non-adjacent nodes are filled in \label{fig:near_a}}
\end{figure}
